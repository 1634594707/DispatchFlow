...
.amap-geo-map {
  /* 这块地图（含 AMap 自己注入的 marker/label/面板节点）永远画在深色浮层上。
     ...
     还把 --fsd-bg-hover 改成了浅色 #edf2f3、把强调/语义色改成深色调
     （accent #438f9b、accent-strong #326f78、error #c45868）—— 于是深色芯片上落深色字：
     marker 标签 38 个全部隐形，L0/L1/L2 与图层面板对比度只剩 ~2.9:1，
     选中档的 L1试点 更只有 2.30:1，悬停态则是浅字压浅底。
  ...
  --fsd-accent: var(--fsd-accent-on-overlay);
  --fsd-accent-strong: var(--fsd-accent-strong-on-overlay);
  --fsd-error: var(--fsd-error-on-overlay);
