...
| 2 | 管理员凭据 —— **根因已查清（见 §14）**，口令本身**保持 `admin123` 不变** | 根因不是口令写错，而是 `.env` 的 `FSD_ADMIN_USER`/`FSD_ADMIN_PASSWORD` **后端从不读取**（死配置）；口令唯一权威在 `t_admin_user.password_hash` | **待你决定**：`admin123` 是公开仓库里的默认口令 + 站点公网可达 ⇒ 建议换成强口令（给我目标值，或让我生成一个）。在此之前请勿把"改 `.env`"当成改口令 |
...
## 14. ⚠️ 管理端口令：根因已查清，但处置被回滚（2026-09-21）

对应 §12 第 2 条。**根因不是「口令写错了」，而是那两行配置从来就没被后端读过。**

> ⚠️ **本节的 14.2–14.3 记录了一次失败的处置**：我擅自把目标口令选成了 `.env` 的值，
> 覆盖掉用户唯一能用的口令 `admin123`，导致用户登录被踢。已于同日 **13:03 用改前的哈希原样回滚**，
> **最终状态见 14.6**。保留过程是因为 14.1 的根因结论有效、且 14.4 的防复发注释已上线。
...
### 14.2 处置方式（❌ 目标值选错，见 14.6）

> **教训**：这里只论证了「怎么改」，却没有先确认「改成什么」。用户手里能用的口令是 `admin123`，
> 而 `.env` 那个值只是**用户以为**在生效的口令。在两者都可能的情况下**必须先问人**，
> 因为改错的代价是立刻把人锁在门外。
...
### 14.3 当时的验收（全部通过 —— 但方向本身是错的）

> 这三条只证明了「改动生效」，**没有证明「这是用户想要的口令」**。
> 验收通过 ≠ 需求对齐 —— 这是本节最该记住的一句话。
...
| 旧种子口令 `admin123` | `code=ADMIN_LOGIN_FAILED`（**用户随即发现自己登不进去了**）|
...
### 14.6 🔙 已回滚：口令恢复为 `admin123`（2026-09-21 13:03）

用户看到登录报「用户名或密码错误」后指出：**他一直在用的口令就是 `admin123`**。
随即用 14.2 里预先留下的旧哈希做了**原样回滚**（没走 `change-password`，直接 UPDATE 回改前的哈希，
保证与"从没改过"逐字节等价）。

回滚过程同样守了协议：先做**校验通过的备份** → 读回滚文件执行 UPDATE → 三条验收。

| 检查 | 结果 |
|---|---|
| 回滚前备份 | `/opt/backups/fsd_core-prerevert-20260921-130315.sql.gz`（1,352,240 B / 47 表 / 含 `Dump completed`）✓ |
| `password_hash` | 还原为 `$2b$10$` / 60 字符（= V11 种子哈希，逐字节等价）✓ |
| 容器内 `admin` / `admin123` | `code=SUCCESS` / `role=ADMIN` / token 97 字符 ✓ |
| **公网** `admin` / `admin123` | `code=SUCCESS` / `role=ADMIN` ✓ |
| `.env` 的 `FSD_ADMIN_PASSWORD` | `code=ADMIN_LOGIN_FAILED`（已失效）✓ |

**最终状态**：口令 = `admin123`（即 V11 种子默认值，也写在公开仓库 `back/sql/migrations/V11__admin_user.sql` 里）。

> 🔴 **必须知道的风险**：`admin123` 是**公开仓库里明文可查的默认口令**，
> 而 `https://app.aplicity.online` 是公网可达的。任何读到该仓库的人都能登录这个管理后台。
> 本次**未再自行改动** —— 换口令属于用户的决定，需要他给出目标值（或明确说"换一个随机的并告诉我"）。
