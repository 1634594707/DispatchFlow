...
- [~] 提交/丢弃其余未提交改动 —— 已按主题提交为 `pre-deploy-v52-56`；**仍按你决定留在工作区未提交的只有两处**：`.gh-check.js`（CI 探针草稿）与未接线的 geo 三件套。**推送仍未做**（本地领先 origin/main，等你决定何时 push）
...
2. ~~生产库 Flyway 实际版本~~ **已答并已追平**：2026-09-21 核得生产在 V51（基线 V50），2026-09-22 第一轮部署已应用到 V56。
