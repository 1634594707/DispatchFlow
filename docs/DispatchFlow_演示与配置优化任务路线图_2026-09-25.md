...
按裁定收到 **12 柜 + 2 个卫星点**：新增 `zjf_retire_extra_swap_cabinets.sql`，把 23 个柜置 `INACTIVE`
（不删行 —— 与设施 v2 同一套做法，留着才能回退，历史会话外键也不悬空）。

**保留哪 12 个不是我手挑**：`scripts/geo/swap_cabinet_placement.py` 的布点本身就是
greedy farthest-point sampling（k-center 的 2-近似，从基地节点起步）**并按被选中顺序编号**，
所以"保留编号前 12 个"= k-center 意义下最分散的 12 个。实测最近邻间距对照：
...
复核：`tmp/pick-cabs.mjs` 对 35 个坐标重跑最远点采样，结果与 01..12 完全一致。

**结果（本机实测）**：ACTIVE 柜 35 → **12**、充电站 8 不变 ⇒ 补能站 **20 行 / 14 个不同位置**
（母港 6 根桩与柜 01..06 同坐标会合并成同一个徽标）。幂等性验过：连灌两次，`remark` 仍是
1 种、23 行、长度 57 字符不变（所以用的是整串覆盖而不是 `CONCAT` 追加）。
`v14` 的设施夹具形态与断言同步改成 12 柜 / 8 充电站（它一直声称"照抄活库"，不改就是假出处）。
