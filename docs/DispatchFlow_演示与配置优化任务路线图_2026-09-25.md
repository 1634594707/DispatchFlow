...
### 15.5 ⚠ 没修完的一条：空闲车占位会衰减（本机与生产同现象）

上线后复量：生产 35 台全部 ONLINE/IDLE、SOC 全部 >30（**"一堆故障车"这个症状消失了**），
但**只有 6 台占着车位，29 台 IDLE 无位**。本机同现象。

我先假设原因是"充电成功时 `releaseByVehicle` 放掉了待命位，而 `ensureStandbyLocation()` 只在
`standbyPoint == null` 时才重新取位 ⇒ 位被放掉后再也不补"，并在两处释放后加了 `state.standbyPoint = null`
（这条改动本身是对的：留着旧引用等于指向一个随时会被别人占走的位。已提交，闸门全绿）。
**但它没有让绑定数涨回来**：本机重启 + 清空车位后跑 5 分钟，仍是 6 台占位、29 台无位。

⇒ 说明真正的分配发生在别处：`shouldPreferCharging()` 为真时车直接进补能路径，
抢到桩就 `releaseByVehicle` 去桩位、充完再进下一轮，**几乎没有一步走到 `routeToStandby`**，
所以那条"重新取位"的分支基本不被执行。
下一步要判的是：这到底是"车都停在桩位/充电站上"（可接受），还是"车停在去桩路上的任意点"（不可接受）——
需要读运行时阶段分布而不是只看车位表。**在判清之前不要动 `idleChargeWhenNoDemand`。**

> 生产当前 = 第 10 轮（`0886b15`）；`standbyPoint=null` 那一行已在仓库、**未部署**，
> 因为它没有改变可观察行为，不值得为它单独再走一次生产发布。
