...
> **2026-06-09 复核状态**：V7-A1 至 V7-A6 已完成 P0 操作闭环；V7-H1 至 V7-H3 已完成健康可信主链路；V7-R1 至 V7-R5、V7-Q1 至 V7-Q4 已完成。
...
- [x] **V7-R1** SSE 连接治理
  - 实现证据：`back/fsd-admin-api/src/main/java/com/fsd/admin/service/impl/AdminDispatchStreamServiceImpl.java:L35-L68` 已有超时、最大连接数、连接数统计与园区广播过滤；`back/fsd-admin-api/src/test/java/com/fsd/admin/service/impl/AdminDispatchStreamServiceImplTest.java` 覆盖超限与园区隔离
  - 已落地：`fsd.admin.sse.timeout-ms`、`fsd.admin.sse.max-connections` 可配置；超过上限抛出 `SSE_CONNECTION_LIMIT_EXCEEDED`；健康指标可读取当前 SSE 连接数；不同园区订阅按 `parkId` 隔离广播
  - 验收：超过连接上限时返回明确错误；系统健康指标可看到当前 SSE 连接数；不同园区订阅不互相收到广播

- [x] **V7-R2** URL query token 治理
  - 实现证据：`back/fsd-bootstrap/src/main/java/com/fsd/bootstrap/config/AdminAuthInterceptor.java:L80-L84` 对 SSE 入口改为 Controller 内短期凭证校验；`front/src/api/dispatch.ts:L65-L84` 使用 `/admin/sse-ticket` 获取短期一次性凭证；`back/fsd-bootstrap/src/main/java/com/fsd/bootstrap/config/OpenApiConfig.java:L24-L28` 已移除长期 query token 描述
  - 已落地：运营分析与历史报表导出已通过 `downloadAnalyticsFile` 走 axios 请求头；SSE URL 仅携带短期 ticket，不携带长期管理 token
...
- [x] **V7-R4** Webhook 异步投递与重试治理
  - 实现证据：`back/fsd-dispatch/src/main/java/com/fsd/dispatch/integration/WebhookDeliveryService.java:L54-L63` 已异步投递并跳过熔断订阅；`back/fsd-dispatch/src/main/java/com/fsd/dispatch/integration/WebhookDeliveryService.java:L68-L133` 已设置 5 秒请求超时、最多 3 次退避重试并记录 delivery log
  - 已落地：单个订阅慢响应不再阻塞订阅循环；失败会记录 delivery log 并累加 failureCount；连续失败达到阈值后熔断跳过
  - 验收：单个订阅 5 秒超时不阻塞其他订阅投递

- [x] **V7-R5** 派单 Redis 锁 TTL 配置化与观测
  - 实现证据：`back/fsd-dispatch/src/main/java/com/fsd/dispatch/infra/impl/RedisDispatchLockService.java:L32-L63` 已使用 `DispatchLockProperties#getTtl`、记录锁冲突并采集锁持有耗时；`back/fsd-dispatch/src/main/java/com/fsd/dispatch/metrics/DispatchLockMetrics.java` 提供失败次数与耗时快照；`back/fsd-admin-api/src/main/java/com/fsd/admin/service/impl/SystemHealthAdminServiceImpl.java:L80-L92` 已接入详细健康指标
  - 已落地：TTL 配置化；锁冲突可通过日志定位；获取锁失败次数、锁持有次数、平均/最大持有耗时可通过健康指标查看
...
- [x] **V7-Q1** P0 操作闭环 E2E 覆盖
  - 实现证据：`front/scripts/e2e/v6-critical-flows.spec.ts` 已覆盖任务派单、任务改派、任务取消、订单取消、异常重新派单，并从页面按钮触发用户路径
...
| **v6.3.0** | + V7-R1、V7-R2、V7-R4、V7-R5 剩余待收口项已完成 |
| **v7.0.0** | + V7-Q1 用户路径 E2E 覆盖已完成；V7-Q2 至 V7-Q4 已完成 |
