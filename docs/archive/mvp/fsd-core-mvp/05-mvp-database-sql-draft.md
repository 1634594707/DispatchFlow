# FSD-Core MVP 数据库表结构 SQL 草案

## 1. 文档说明

- 文档目标：提供 `FSD-Core MVP` 第一阶段核心表的建表草案，供后端开发、数据库设计评审、接口实现使用。
- 文档范围：仅覆盖 `订单`、`调度任务`、`车辆`、`调度任务操作日志`、`调度异常记录`。
- 数据库建议：`MySQL 8.0`

## 2. 设计原则

- `MySQL` 为最终业务真相源
- 所有核心业务表均保留 `created_at`、`updated_at`
- 关键状态更新建议带 `version` 乐观锁字段
- 逻辑删除统一使用 `deleted`
- 状态字段使用字符串枚举，避免魔法数字
...
- 若业务确认 `source_type + external_order_no` 必须全局幂等，可追加联合唯一索引
- `dispatch_task_id` 允许为空，直到调度任务生成成功

### 3.2 调度任务表 `t_dispatch_task`
...
- MVP 阶段默认一个订单只对应一个主调度任务
- 若后续支持补派或拆分子任务，应新增扩展模型，不建议现在提前复杂化
...
- `online_status` 与 `dispatch_status` 必须拆开，不能混成一个字段
- `last_report_time` 用于离线判定和旧数据覆盖保护

### 3.4 调度任务操作日志表 `t_dispatch_task_operate_log`
...
### 3.5 调度异常记录表 `t_dispatch_exception_record`
...
- 订单查询高频按 `status + created_at`
- 调度任务查询高频按 `status + created_at`
- 车辆候选筛选高频按 `online_status + dispatch_status`
- 异常查询高频按 `exception_type + exception_status`

说明：

- MVP 阶段先按主查询路径建索引，不做过度优化
- 真正的联合索引应在接口和 SQL 明确后再复审

## 6. 后续落地建议

- 将本草案转成正式 `V1__init.sql` 建表脚本
- 与后端查询接口一起复核索引
- 在编码前固定状态枚举和字段命名
