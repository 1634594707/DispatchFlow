...
| 优先级 | 里程碑 | 用户感知 |
|--------|--------|----------|
| **P0-1** | **V4-S** OSM 站点统一 + 校验收紧 | Marker 在道路上；典型订单路线稳定 |
| **P0-2** | **V4-W** 工作台园区态势收敛 | 态势图只显示 **运营相关** 少量点，可读、像真实园区 |
| **P0-3** | **V4-M** 移动下单重构 | 像外卖：选线路 → 下单 → 跟车；无 Key 干扰 |
| **P0-4** | **V4-R7** M8-R7 贴路验收关闭 | 脚本 + 录屏；可发 v3.1.0 |

**P1**：充电类型与回充策略 · 运营链路（V3 M9-O）· 管理端配置自检  
**P2**：演示模式 · JaCoCo · 工程 polish
...
| **TD-P0-6** | 工作台态势全量站点 | **V4-W** `ParkMiniMap` 过滤 |
| **TD-P0-7** | 移动页站点/UX 过载 | **V4-M** 重构 |
...
- [ ] **V4-E1** `StationType.CHARGING_STATION` + 桩位元数据
- [ ] **V4-E2** 地理图电量：tooltip / 低电橙闪 / 危急红闪
- [ ] **V4-E3** 调度图电量迷你图标
- [ ] **V4-E4** `Tracking.vue` 车队电量面板 + 筛选低电
- [ ] **V4-E5** 充电站地图插头图标 + 桩位占用
- [ ] **V4-E6** 管理端充电站 CRUD + L1 贴路校验
- [ ] **V4-E7** 回充：最近可达 `ZJF-CHG-*`（非 index 轮询）
...
- [ ] **V4-D1** 管理端配置自检页（JS / Web / 移动 Key + health）
- [ ] **V4-D2** 园区调度场景 tooltip「内部路网示意，非真实道路」
- [ ] **V4-D3** 短驳地理顶栏「当前：叠石桥 L1 试点」
- [ ] **V4-D4** Key 缺失引导统一 → 自检页
- [ ] **V4-D5** 找家纺品牌叙事（登录/工作台/大屏）
- [ ] **V4-D6** 演示官方入口文档化（ParkOverview · mobile · geo 链接）
...
- [ ] **V4-D13** 恢复 `.env.example` · `front/README.md` 同步
...
- [ ] **V4-Q4** 生产去除 `sseClient.ts` 调试 log
...
| 版本 | 条件 |
|------|------|
| **v3.1.0** | V4-S + V4-W + V4-M（M1–M6）+ V4-R7 通过 |
| **v4.0.0** | 上列 + V4-E 核心 + V4-O + V4-D1 自检页 + 文档四源一致 |
